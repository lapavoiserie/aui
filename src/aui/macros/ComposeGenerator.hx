	...
	**/
	static function generateGlanceWidget(packageName:String, jvmApp:String):Void {
		var lines = [
			...
			"        provideContent {",
			"            // The container is not decoration. A widget draws over",
			"            // whatever wallpaper the user chose, so a tree with no",
			"            // background and no colour of its own is black text on an",
			"            // unknown ground — present, findable by an automation",
			"            // harness, and invisible to a human. appWidgetBackground()",
			"            // is also what lets the launcher round our corners.",
			"            GlanceTheme {",
			"                Column(",
			...
			"                    if (json == null) {",
			"                        // Placed but never pushed to: the application",
			"                        // has not run since. Say so rather than draw a",
			"                        // blank.",
			"                        Text(\"…\", style = onSurface())",
			"                    } else {",
			...
			"    }",
			"",
			"    // Every piece of text takes the theme's foreground: the default is",
			"    // black, which is unreadable on half the wallpapers in the world.",
			"    @Composable",
			"    private fun onSurface() = TextStyle(color = GlanceTheme.colors.onSurface)",
			"",
			"    /**",
			...
			"            \"Text\" -> Text(props?.optString(\"text\") ?: \"\", style = onSurface())",
			"            // A Button is a label plus an ACTION ID — the closure",
			"            // never crossed and never could. Tapping sends the id",
			"            // back to AuiGlanceAction, which resolves it against the",
			"            // table and runs the closure in our own process.",
			"            \"Button\" -> {",
			"                val label = props?.optString(\"label\") ?: \"\"",
			"                val actionId = node.optJSONObject(\"actions\")?.optInt(\"onClick\", -1) ?: -1",
			"                if (actionId < 0) {",
			"                    // Declared without one: it draws, it does not act.",
			"                    Text(label, style = onSurface())",
			"                } else {",
			"                    Button(",
			"                        text = label,",
			"                        onClick = actionRunCallback<AuiGlanceAction>(",
			"                            actionParametersOf(AuiGlanceAction.ACTION_ID to actionId)",
			"                        )",
			"                    )",
			"                }",
			"            }",
			...
			"            else -> Text(\"?\" + type, style = onSurface())",
			"        }",
			"    }",
			"",
			"    @Composable",
			...
			"     *",
			"     * The running one first; then the one this process already",
			"     * sampled, which holds what the user last saw and any tap that",
			"     * has since landed; and only then a fresh one, whose state is",
			"     * the application's initial state, honestly.",
			"     */",
			"    fun sample(): String? =",
			"        aui.mui.GlanceBridge.sampleLive()",
			"            ?: aui.mui.GlanceBridge.sampleAgain()",
			"            ?: aui.mui.GlanceBridge.sampleOf(" + jvmApp + "())",
			"",
			"    suspend fun push(context: Context) {",
			"        val json = sample() ?: return",
			...
			"",
			"/**",
			" * What a tap on a widget button does.",
			" *",
			" * The launcher hands back an ID, never a closure — it could not hold",
			" * one, and this callback runs in OUR process, which may have started",
			" * for this very tap. So: sample first, which rebuilds the action",
			" * table (ids are keyed by place, so the same button in the same slot",
			" * keeps its id), then invoke, then push the picture the action just",
			" * changed.",
			" */",
			"class AuiGlanceAction : ActionCallback {",
			"    companion object {",
			"        val ACTION_ID = ActionParameters.Key<Int>(\"aui.glance.action\")",
			"    }",
			"",
	...
	/**
		Write `AuiFonts.kt`: the families this application ships, by name.

		A Compose `FontFamily` is built from a file, and a tree names a family.
		Nothing on the device can turn one into the other -- Android will not
		tell you what family a `.ttf` in your assets declares -- so the mapping
		is made here, where the file can be read, and shipped as a table.

		A family with several faces keeps them all: Compose picks the weight and
		the slant it needs from the family, which is what a font family is for.
	**/
	static function writeFontTable(runtimeDir:String):Void {
		var faces = shippedFaces();
		var out = new StringBuf();
		out.add("package aui.runtime\n\n");
		out.add("import androidx.compose.ui.text.font.Font\n");
		out.add("import androidx.compose.ui.text.font.FontFamily\n");
		out.add("import androidx.compose.ui.text.font.FontStyle\n");
		out.add("import androidx.compose.ui.text.font.FontWeight\n\n");
		out.add("/**\n");
		out.add(" * The fonts this application ships, by the family name they declare.\n");
		out.add(" *\n");
		out.add(" * Generated by aui's build from the files in assets/fonts: a family name\n");
		out.add(" * lives in a font's own tables, and nothing on the device reads it back\n");
		out.add(" * out of an asset. A name this application does not ship answers null,\n");
		out.add(" * and the text is set in the platform's own font -- which is what a tree\n");
		out.add(" * from elsewhere naming an unknown family must do.\n");
		out.add(" */\n");
		out.add("object AuiFonts {\n");
		out.add("    private val built = HashMap<String, FontFamily>()\n\n");
		out.add("    private val shipped: Map<String, List<Triple<String, Int, Boolean>>> = mapOf(\n");
		var byFamily = new Map<String, Array<{path:String, weight:Int, italic:Bool}>>();
		var order = [];
		for (face in faces) {
			if (!byFamily.exists(face.family)) {
				byFamily.set(face.family, []);
				order.push(face.family);
			}
			byFamily.get(face.family).push({path: face.path, weight: face.weight, italic: face.italic});
		}
		for (family in order) {
			var entries = [
				for (face in byFamily.get(family))
					'Triple("' + face.path + '", ' + face.weight + ', ' + (face.italic ? "true" : "false") + ')'
			];
			out.add('        "' + family.toLowerCase() + '" to listOf(' + entries.join(", ") + '),\n');
		}
		out.add("    )\n\n");
		out.add("    /** The family of that name, or null when this application ships none. */\n");
		out.add("    fun family(name: String): FontFamily? {\n");
		out.add("        if (name.isEmpty()) return null\n");
		out.add("        val key = name.lowercase()\n");
		out.add("        built[key]?.let { return it }\n");
		out.add("        val faces = shipped[key] ?: return null\n");
		out.add("        val made = FontFamily(faces.map { (path, weight, italic) ->\n");
		out.add("            Font(path, AndroidContext.application!!.assets,\n");
		out.add("                weight = FontWeight(weight),\n");
		out.add("                style = if (italic) FontStyle.Italic else FontStyle.Normal)\n");
		out.add("        })\n");
		out.add("        built[key] = made\n");
		out.add("        return made\n");
		out.add("    }\n}\n");
		File.saveContent(runtimeDir + "/AuiFonts.kt", out.toString());
	}

	/** The font files under the application's assets, as their tables describe them. **/
	static function shippedFaces():Array<{path:String, family:String, weight:Int, italic:Bool}> {
		var out = [];
		...
		for (name in names) {
			...
			var face = nui.FontFile.read(try sys.io.File.getBytes(path) catch (_:Dynamic) null);
			if (face == null) continue;
			out.push({path: "fonts/" + name, family: face.family, weight: face.weight, italic: face.italic});
		}
		return out;
	}
	...
	static function writeAuiRuntimeKotlin():Void {
		...
		if (RenderPath.isDynamic()) {
			...
			// The fonts this application ships, as a table the renderer reads:
			// a family name to the file that carries it. The name is in the
			// file (`nui.FontFile`), and Compose needs the file -- so the two
			// have to be put together by whoever can read both, which is this
			// build and not the device.
			writeFontTable(runtimeDir);
