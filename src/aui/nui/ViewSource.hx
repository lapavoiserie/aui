...
class ViewSource implements NodeSource<View> {
	...
	**/
	...
	/**
		...
		moved. A list whose rows move says otherwise, row by row -- and
		`DynamicComposable.kt` has been reading it as `nodeId`, for Compose's
		`key()`, since before an app could set one.
	**/
